...
$secrets = require '/usr/www/users/sabgaekbnf/config/secrets.php';
...
if (
    empty($host) || empty($db_user) || empty($db_password) || empty($dbname) ||
    empty($email_host) || empty($email_user) || empty($email_password)
) {
    error_log("FATAL: Missing secrets for cron job");
    exit(1);
}
...
// Error log file
$logFile = '2026_cron_log.txt'; // Adjust this path as needed
   ...
   // Loop to check emails every 5 minutes within the 2-hour cron job
    ...
    if ($emails) {
            ...
            if ($fixture['Completed'] == 0) {
                ...
                if ($success) {
                    ...
                    log_debug("Fixture marked as completed");
                     ...
                     // === Trigger Python stats refresh ===
                    $series_stmt = $conn->prepare("
                        SELECT s.SeriesID 
                        FROM SeriesMatchTypes s 
                        JOIN Fixtures f ON f.MatchTypeID = s.MatchTypeID 
                        WHERE f.FixtureID = ?
                    ");
                    $series_stmt->bind_param("i", $fixture['FixtureID']);
                    $series_stmt->execute();
                    $series_result = $series_stmt->get_result();
                
                    while ($row = $series_result->fetch_assoc()) {
                        $series_id = $row['SeriesID'];
                        log_debug("Triggering refresh for SeriesID: $series_id");
                
                        $cmd = escapeshellcmd("/usr/bin/python3 /usr/www/users/sabgaekbnf/cron/refresh_series_for_cron.py $series_id");
                        exec($cmd, $output, $return_var);
                        log_debug("Refresh script return: $return_var | Output: " . implode(" | ", $output));
                        
                        $cmd = escapeshellcmd("/usr/bin/python3 /usr/www/users/sabgaekbnf/cron/refresh_matchtype_stats.py $match_type_id");
                        exec($cmd, $output, $return_var);
                        log_debug("MatchType refresh: $return_var | Output: " . implode(" | ", $output));
                        
                        $cmd = escapeshellcmd("/usr/bin/python3 /usr/www/users/sabgaekbnf/cron/refresh_remaining_by_series.py $series_id");
                        exec($cmd, $output, $return_var);
                        log_debug("SeriesRemainingFixturesCache refresh: $return_var | Output: " . implode(" | ", $output));
       ...
       // Sleep for 5 minutes before checking again
        sleep(300); // 5 minutes
        if ($conn->ping() === false) {
            log_debug("Reconnecting to the database...");
            $conn = new mysqli($host, $db_user, $db_password, $dbname);
            if ($conn->connect_error) {
                log_debug("Database reconnection error: " . $conn->connect_error);
                exit;
    }
}
    }
